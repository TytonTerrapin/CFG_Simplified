import { remove_null_productions, remove_unit_productions, remove_useless_symbols, _calculate_metrics, check_membership } from './logic.js';
...
const DOM = {
    ...
    // Membership Checker
    membershipInput: document.getElementById('membership-input'),
    btnCheckMembership: document.getElementById('btn-check-membership'),
    membershipResult: document.getElementById('membership-result'),
    membershipStatus: document.getElementById('membership-status'),
    membershipDerivationContainer: document.getElementById('membership-derivation-container'),
    membershipDerivationTrace: document.getElementById('membership-derivation-trace'),
};
...
function flattenPipelineSteps() {
    ...
    steps.push({
        stage: 'Membership', stageIdx: 5, color: '#8b5cf6', action: 'membership',
        title: 'Grammar Membership Checker',
        rule: 'Test a String',
        reasoning: 'Verify if a string belongs to the language generated by this simplified grammar.',
        grammarBefore: deepCopy(runningGrammar), grammarAfter: deepCopy(runningGrammar),
    });
...
function initSbsCarousel() {
    ...
    const cards = sbsSteps.map((step, idx) => {
        ...
        if (step.action === 'membership') {
            return `<div class="sbs-step sbs-action-membership" id="sbs-step-card-${idx}" style="--step-color: ${step.color}; --step-color-rgb: ${colorRgb};">
                <div class="sbs-step-badge" style="background: ${step.color}22; color: ${step.color};">TEST</div>
                <div class="sbs-membership-form" style="margin: 15px 0;">
                    <input type="text" id="sbs-membership-input" placeholder="Enter string (e.g. aabb)" class="builder-input" style="width: 100%; margin-bottom: 10px; text-align: center;">
                    <button class="btn btn-primary" id="btn-sbs-check-membership" style="width: 100%;">Check Membership</button>
                </div>
                <div id="sbs-membership-result" class="hidden" style="margin-top: 15px; font-size: 0.9rem; text-align: center;">
                    <div id="sbs-membership-status" style="font-weight: bold; margin-bottom: 5px;"></div>
                    <div id="sbs-membership-trace" style="display: flex; flex-wrap: wrap; gap: 4px; justify-content: center; font-family: 'Fira Code', monospace; padding: 10px; background: rgba(0,0,0,0.2); border-radius: 4px;"></div>
                </div>
    ...
    // Add event listener for SBS membership check
    const btnSbsCheck = document.getElementById('btn-sbs-check-membership');
    if (btnSbsCheck) {
        btnSbsCheck.addEventListener('click', () => {
            const input = document.getElementById('sbs-membership-input');
            const resultDiv = document.getElementById('sbs-membership-result');
            const statusDiv = document.getElementById('sbs-membership-status');
            const traceDiv = document.getElementById('sbs-membership-trace');
            
            const target = input.value.trim();
            const finalGrammar = sbsSteps[sbsSteps.length - 1].grammarAfter;
            ...
            setTimeout(() => {
                const derivation = check_membership(finalGrammar, target);
                if (derivation) {
                    statusDiv.innerHTML = `<span class="membership-accepted">ACCEPTED</span>`;
                    derivation.forEach((step, idx) => {
                        setTimeout(() => {
                            if (idx > 0) {
                                const arrow = document.createElement('span');
                                arrow.className = 'derivation-arrow';
                                arrow.style.fontSize = '1rem';
                                arrow.innerText = '⇒';
                                traceDiv.appendChild(arrow);
                            }
                            const stepEl = document.createElement('div');
                            stepEl.className = 'derivation-step';
                            stepEl.style.padding = '4px 8px';
                            stepEl.style.fontSize = '0.9rem';
                            stepEl.innerText = step === '' ? 'ε' : step;
                            traceDiv.appendChild(stepEl);
                        }, idx * 300);
                    });
                } else {
                    statusDiv.innerHTML = `<span class="membership-rejected">REJECTED</span>`;
                }
            }, 300);
        });
    }
...
DOM.btnCheckMembership.addEventListener('click', () => {
    if (!pipelineHistory.length) {
        alert("Please run the simplification pipeline first!");
        return;
    }

    const target = DOM.membershipInput.value.trim();
    const finalGrammar = pipelineHistory[pipelineHistory.length - 1].grammar;
    
    // Clear previous results
    DOM.membershipResult.classList.remove('hidden');
    DOM.membershipStatus.innerHTML = '<span style="color: #888;">Checking...</span>';
    DOM.membershipDerivationContainer.classList.add('hidden');
    DOM.membershipDerivationTrace.innerHTML = '';
    ...
    setTimeout(() => {
        ...
        if (derivation) {
            DOM.membershipStatus.innerHTML = `<span class="membership-accepted">ACCEPTED</span> — The string "${target || 'ε'}" belongs to the language.`;
            DOM.membershipDerivationContainer.classList.remove('hidden');
            
            // Animate derivation trace
            derivation.forEach((step, idx) => {
                setTimeout(() => {
                    if (idx > 0) {
                        const arrow = document.createElement('span');
                        arrow.className = 'derivation-arrow';
                        arrow.innerText = '⇒';
                        DOM.membershipDerivationTrace.appendChild(arrow);
                    }
                    
                    const stepEl = document.createElement('div');
                    stepEl.className = 'derivation-step';
                    stepEl.innerText = step === '' ? 'ε' : step;
                    DOM.membershipDerivationTrace.appendChild(stepEl);
                    
                    // Auto scroll the trace container
                    DOM.membershipDerivationTrace.scrollLeft = DOM.membershipDerivationTrace.scrollWidth;
                }, idx * 400);
            });
        } else {
            DOM.membershipStatus.innerHTML = `<span class="membership-rejected">REJECTED</span> — The string "${target}" cannot be generated by this grammar.`;
        }
    }, 500);
});
